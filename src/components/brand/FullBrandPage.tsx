...
export const FullBrandPage = ({ 
  ...
  const renderSection = useCallback((sectionId: SectionId) => {
    ...
    switch (sectionId) {
      ...
      case 'webinars': return <WebinarSeriesSection webinars={brand.webinars || []} onWebinarsChange={editHandler((webinars) => onBrandUpdate({ webinars }))} customSubtitle={customSubtitle} onSubtitleChange={onSubtitleChange} entityName={brand.hero?.name || brand.name} entityType="brand" industry={(brand as any).industry} websiteUrl={brand.websites?.[0]?.url} />;
      ...
      case 'imagery': return <ImagerySection imagery={brand.imagery} onImageryChange={editHandler((imagery) => onBrandUpdate({ imagery }))} customSubtitle={customSubtitle} onSubtitleChange={onSubtitleChange} entityId={brandId} entityType={entityType as 'brand' | 'product' | 'event' || 'brand'} isAdmin={canEdit} />;
