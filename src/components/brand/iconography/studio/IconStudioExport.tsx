...
export const IconStudioExport = ({
  ...
  const exportPngSprite = async () => {
    // Generate a CSS sprite sheet with inline SVGs rendered to canvas
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    const iconCount = allIcons.length;
    const maxSize = Math.max(...pngSizes);
    const cols = Math.ceil(Math.sqrt(iconCount));
    const padding = 4;

    // Generate one sheet per size
  ...
  const exportIconFont = () => {
    // Generate CSS file with SVG-based icon classes
