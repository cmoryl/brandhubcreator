...
import { Menu, LayoutList, ScrollText, ArrowLeft, Lock, Shield, LogOut, Star, Calendar, Building2, Brain, Settings, Download } from 'lucide-react';
  ...
  const renderSectionContent = useCallback((sectionId: EventSectionId): React.ReactNode => {
    ...
    switch (sectionId) {
      case 'hero': 
        ...
        return <HeroSection hero={event.hero} onHeroChange={editHandler((hero) => updateEvent({ hero }))} onOpenIntelligence={canEdit ? () => setIntelligenceOpen(true) : undefined} guideData={event as unknown as Record<string, unknown>} entityType="event" entityId={event.id} />;
      case 'eventdetails':
