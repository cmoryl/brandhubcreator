      ...
      case 'socialassets': 
        return (
          <SocialAssetsSection
            ...
            onSocialAssetsChange={(socialAssets) => updateEvent({ socialAssets })}
            displayBanners={event.displayBanners || []}
            onDisplayBannersChange={(displayBanners) => updateEvent({ displayBanners })}
          />
        );
      case 'eventsponsors':
          ...
          <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-auto">
            <div className={`${getContentWidthClass()} mx-auto animate-fade-in-up ${getSectionSpacingClass()}`}>
              ...
              {viewMode === 'sections' ? (
                ...
                <div className={getSectionSpacingClass()}>
                  {(isGuideAdmin ? sectionOrder : sectionOrder.filter(id => !hiddenSections.includes(id))).map((sectionId) => (
                    <div key={sectionId} id={sectionId} className="scroll-mt-24">
                      {(() => {
                        switch (sectionId) {
                          case 'hero': 
