...
const EventEditor = () => {
  ...
  const renderSectionContent = useCallback((sectionId: EventSectionId): React.ReactNode => {
    ...
    switch (sectionId) {
      ...
      case 'iconography': return <IconographySection iconography={event.iconography} onIconographyChange={editHandler((iconography) => updateEvent({ iconography }))} brandColors={event.colors?.map(c => ({ hex: c.hex, name: c.name })) || []} organizationId={organization?.id} brandId={event.id} entityType="event" entityName={event.hero?.name || event.name} />;
