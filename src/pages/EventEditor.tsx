...
const EventEditor = () => {
  ...
  const { counts: externalCounts } = useExternalSectionCounts(event?.id, 'event', eventRefreshTrigger);
  ...
  const renderSectionContent = useCallback((sectionId: EventSectionId): React.ReactNode => {
    ...
    switch (sectionId) {
      ...
      case 'webinars': return <WebinarSeriesSection webinars={(event as any).webinars || []} onWebinarsChange={editHandler((webinars) => updateEvent({ webinars } as any))} entityName={event.hero?.name || event.name} entityType="event" industry={(event as any).industry} websiteUrl={(event as any).websites?.[0]?.url} />;
