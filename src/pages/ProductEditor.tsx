...
const ProductEditor = () => {
  ...
  const renderSection = () => {
    ...
    switch (activeSection) {
      ...
      case 'iconography': return <IconographySection iconography={currentProduct.iconography} onIconographyChange={editHandler((iconography) => handleUpdateProduct({ iconography }))} brandColors={currentProduct.colors?.map(c => ({ hex: c.hex, name: c.name })) || []} organizationId={organization?.id} brandId={currentProduct.id} entityType="product" entityName={currentProduct.hero?.name || currentProduct.name} />;
