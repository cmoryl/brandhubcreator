...
.card{background:#161b22;border:1px solid #30363d;border-radius:12px;padding:32px 40px;max-width:480px;text-align:center;}
h1{color:#00c853;margin:0 0 12px;font-size:20px;}p{color:#8b949e;margin:0 0 18px;line-height:1.5;}
a{display:inline-block;background:#00c853;color:#001a0a;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:700;}</style></head>
<body><div class="card"><h1>${title}</h1>${body}<a href="${dest}">Return to audit page</a></div>
<script>setTimeout(()=>{location.href=${JSON.stringify(dest)};},2500);</script></body></html>`,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (error) return htmlResponse('Canva connection cancelled', `<p>${error}</p>`);
    if (!code) return htmlResponse('Missing authorization code', '<p>Canva did not return a code.</p>');
    ...
    if (!tokenRes.ok) {
      ...
      return htmlResponse('Canva token exchange failed', `<p>${tokenRes.status}: ${tokenText.slice(0, 300)}</p>`, returnTo);
    }
