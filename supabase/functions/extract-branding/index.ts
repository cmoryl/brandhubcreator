...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.log('[extract-branding] Authentication failed: No authorization header');
      return new Response(
    ...
    if (authError || !user) {
      console.log('[extract-branding] Authentication failed: Invalid user', authError?.message);
      return new Response(
    ...
    console.log(`[extract-branding] Authenticated user: ${user.id}`);
