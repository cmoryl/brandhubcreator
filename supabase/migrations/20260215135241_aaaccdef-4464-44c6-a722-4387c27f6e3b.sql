...
-- Update get_entity_text_context to also pull from dedicated website_analysis_reports table
...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    -- Website analysis reports: merge from BOTH guide_data insights AND dedicated table
    'website_analyses', COALESCE(
      -- Prefer dedicated DB table reports (full data)
      v_db_analyses,
      -- Fallback to legacy insights-based reports
      (SELECT jsonb_agg(jsonb_build_object(
