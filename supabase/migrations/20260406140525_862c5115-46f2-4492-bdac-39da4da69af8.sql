...
BEGIN
  ...
  -- Fetch persisted website analysis reports
  SELECT jsonb_agg(jsonb_build_object(
    'url', sub.website_url,
    'score', sub.overall_score,
    'grade', sub.grade,
    'summary', left(COALESCE(sub.summary, ''), 300),
    'report_data', sub.report_data
  ))
  INTO v_db_analyses
  FROM (
  ...
  -- Fetch booth AI analyses
  SELECT jsonb_agg(jsonb_build_object(
    'division_name', sub.division_name,
    'variant', sub.variant_label,
    'overall_score', sub.overall_score,
    'summary', sub.summary,
    'design_score', sub.design_score,
    'messaging_score', sub.messaging_score,
    'content_score', sub.content_score,
    'differentiation_score', sub.differentiation_score,
    'engagement_score', sub.engagement_score,
    'production_score', sub.production_score,
    'regional_insights', sub.regional_insights,
    'top_strengths', sub.top_strengths,
    'top_improvements', sub.top_improvements,
    'analyzed_at', sub.created_at
  ))
  INTO v_booth_analyses
  FROM (
    SELECT 
      COALESCE(bcd.name, baa.division_id) AS division_name,
      baa.variant_label,
      baa.overall_score,
  ...
  v_result := jsonb_build_object(
    ...
    'external_assets', (
      SELECT jsonb_agg(jsonb_build_object(
        'title', COALESCE(b->>'title', 'Untitled'),
        'category', b->>'category',
        'url', b->>'externalUrl',
        'source', 'brochures'
      ))
      FROM jsonb_array_elements(COALESCE(v_guide->'brochures', '[]'::jsonb)) AS b
      WHERE b->>'externalUrl' IS NOT NULL AND length(b->>'externalUrl') > 5
      LIMIT 10
    ),
    'external_templates', (
      SELECT jsonb_agg(jsonb_build_object(
        'title', COALESCE(t->>'title', t->>'name', 'Template'),
        'category', t->>'category',
        'url', t->>'externalUrl',
        'source', 'templates'
      ))
      FROM jsonb_array_elements(COALESCE(v_guide->'templates', '[]'::jsonb)) AS t
      WHERE t->>'externalUrl' IS NOT NULL AND length(t->>'externalUrl') > 5
      LIMIT 10
    ),
    'external_presentations', (
      SELECT jsonb_agg(jsonb_build_object(
        'title', COALESCE(pt->>'title', pt->>'name', 'Presentation'),
        'url', pt->>'externalUrl',
        'source', 'presentations'
      ))
      FROM jsonb_array_elements(COALESCE(v_guide->'presentationTemplates', '[]'::jsonb)) AS pt
      WHERE pt->>'externalUrl' IS NOT NULL AND length(pt->>'externalUrl') > 5
      LIMIT 10
    ),
    -- NEW: Internal storage asset URLs for content extraction pipeline
    'internal_asset_urls', (
      SELECT jsonb_agg(sub.item) FROM (
        SELECT jsonb_build_object(
          'title', COALESCE(b->>'title', 'Brochure'),
          'url', b->>'fileUrl',
          'source', 'brochures'
        ) AS item
        FROM jsonb_array_elements(COALESCE(v_guide->'brochures', '[]'::jsonb)) AS b
        ...
        UNION ALL
        SELECT jsonb_build_object(
          'title', COALESCE(cs->>'title', cs->>'name', 'Case Study'),
          'url', cs->>'fileUrl',
          'source', 'caseStudies'
        )
        FROM jsonb_array_elements(COALESCE(v_guide->'caseStudies', '[]'::jsonb)) AS cs
