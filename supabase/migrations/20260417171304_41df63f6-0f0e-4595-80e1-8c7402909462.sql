...
BEGIN
  ...
  -- Visual Direction (do/dont) samples with description
  SELECT jsonb_agg(jsonb_build_object(
    'url', i->>'url',
  ...
  -- Approved Imagery library samples (flatten sections.images)
  SELECT 
  ...
  -- Digital Collateral thumbnails (brochures + presentations + templates + case studies)
  ...
  v_result := jsonb_build_object(
    ...
    -- NEW: Sampled imagery URLs the AI can visually inspect
