ALTER TABLE public.dataforce_compliance_jobs
  ADD COLUMN IF NOT EXISTS weighted_by jsonb;
...
BEGIN
  ...
  -- NEW: Top recent Canva audit findings, so generators bias against flagged issues.
  IF p_table = 'brands' THEN
    SELECT jsonb_agg(f) INTO v_canva_findings FROM (
      SELECT jsonb_build_object(
        'severity', finding->>'severity',
        'type', finding->>'type',
        'title', finding->>'title',
        'description', left(COALESCE(finding->>'description',''), 200),
        'recommendation', left(COALESCE(finding->>'recommendation',''), 200)
      ) AS f
      FROM canva_audit_analyses ca,
           jsonb_array_elements(COALESCE(ca.findings, '[]'::jsonb)) AS finding
      WHERE ca.brand_id = p_id
        AND ca.last_analyzed_at = (
          SELECT MAX(ca2.last_analyzed_at) FROM canva_audit_analyses ca2 WHERE ca2.brand_id = p_id
        )
      ORDER BY CASE LOWER(COALESCE(finding->>'severity',''))
  ...
  v_result := jsonb_build_object(
    ...
    'canva_audit_findings', COALESCE(v_canva_findings, '[]'::jsonb),
